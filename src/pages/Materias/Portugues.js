...
import portuguesWallpaper from '../../imagens/portuguesWallpaper.jpg';

const PortuguesPage = () => {
  const { portugues } = lessonsData;
    ...
    <PageLayout>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', margin: '2% 0' }}>
          ...
          >
            <img src={portuguesWallpaper} alt="Português Wallpaper" style={{ width: '70vh', height: '45vh' }} />
          </div>
        </div>
        <Section title="Português">
          <div className="row row-cols-1 row-cols-md-3 g-4" style={{ justifyContent: 'center' }}>
            {portugues.lessons.map((lesson, index) => (
